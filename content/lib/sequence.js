...
async function pollingEnd() {
//             ------------
  currentNumber = assignCurrentNumber( -1 );              // Проверяем отсутствие записей с незавершёнными запросами
  if ( currentNumber < 0 ) {                              // Если необработанных записей в структуре запросов нет
    ...
    if ( !userIntrusion ) {                               // Если пользователь не вмешивался в ход опроса
      pFinish = drawPollingTime( 'pollingFinish' );                 // Отрисовка времени завершения опроса
      if ( pStart )                                                 // Если опрос проводился, то...
        drawPollingDuration( pStart, pFinish, 'pollingFinish' );    // ...вычисляем и отображаем его продолжительность
      do { // Если активирована задержка запросов к провайдеру, ждём её завершения
        await sleep ( Delay );
      } while ( (pollingCycle.findIndex( function( item ) { return (item.inDelay) ? true : false } )) >= 0 );
      if ( !poolingWinAlive ) {                           // Если в настройках не указано оставлять окно открытым...
        dbMB.close();                                     // ...закрываем базу ('BalanceHistory')...
        console.log( `[MB] IndexedDB '${dbMB.name}' closed` );
        provider.forEach( (item, index) => {
          if (item.pullingTab >= 0) {
            chrome.tabs.remove( item.pullingTab );        // ...закрываем рабочие вкладки для запросов к провайдерам...
            item.pullingTab = -1;
          }
        });
        await chrome.tabs.remove( workTab );              // ...закрываем рабочую вкладку результатов опроса
      }
    }
  }
