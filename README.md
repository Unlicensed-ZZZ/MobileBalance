...
| Плагин | Описание |
| ------ | -------- |
| МТС v3 (API) | <i>Запрос данных оператора связи МТС (через API 2025) с авторизацией (1) по паролю, (2) при пустом пароле - по коду из SMS (однократно на вкладке в ходе запроса)</i><br><b>Забирает:</b><br>- баланс;<br>- кредитный лимит (если он есть);<br>- кэшбэк (если он есть, в поле 'Баланс2');<br>- остаток пакета минут;<br>- остаток пакета SMS;<br>- остаток пакета интернета;<br>- ФИО владельца;<br>- наименование тарифа;<br>- дату завершения оплаченного периода;<br>- статус блокировки;<br>- состав услуг ( формат: 'бесплатные'&nbsp;/ 'платные'&nbsp;/ (сумма по платным) ) |
